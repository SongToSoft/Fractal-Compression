...
namespace Fractal_Compression
{
    class Program
    {
        ...
        static double Shift(Bitmap Image, int range_x, int range_y, int domain_x, int domain_y, int rotate)
        {
            double shift = 0;
            double RangeValue = 0;
            ...
            for (int i = 0; i < range_size; ++i)
            {
                ...
                {
                    RangeValue = RangeValue + (GetValue(RangeImage.GetPixel(range_y + i, range_x + j)));
                    DomainValue = DomainValue + (GetValue(DomainImage.GetPixel(domain_y + i * 2, domain_x + j * 2)));
                }
            }
            //shift = (RangeValue) - 0.75 * (DomainValue); 
            //Будем брать среднее арифметическое
            shift = (RangeValue) / (range_size) - 0.75 * (DomainValue) / (range_size);
            return shift;
        }
        static double Distance(Bitmap Image, int range_x, int range_y, int domain_x, int domain_y, double shift, int rotate)
        {
            ...
            for (int i = 0; i < range_size; ++i)
            {
                for (int j = 0; j < range_size; ++j)
                {
                    RangeValue = GetValue(RangeImage.GetPixel(range_y + i, range_x + j));
                    DomainValue = GetValue(DomainImage.GetPixel(domain_y + i * 2, domain_x + j * 2));
        ...
        static void Compresson(string filename)
        {
                    ...
                    for (int k = 0; k < domain_width; ++k)
                    {
                        for (int l = 0; l < domain_heigth; ++l)
                        {
                            for (int p = 0; p < 4; ++p)
                            {
                                ...
                                current_shift = Shift(Image, RangeBlock.X, RangeBlock.Y, DomainBlock.X, DomainBlock.Y, p);
                ...
                for(int i = 0; i < range_width; ++i)
                {
                    for (int j = 0; j < range_heigth; ++j)
                    {
                        ...
                        for (int k = 0; k < range_size; ++k)
                        {
                            ...
                            {
                                int R = (int)(0.75 * NewDomainImage.GetPixel(k + i * l0, l + j * k0).A + q0);
                                int G = (int)(0.75 * NewDomainImage.GetPixel(k + i * l0, l + j * k0).B + q0);
                                int B = (int)(0.75 * NewDomainImage.GetPixel(k + i * l0, l + j * k0).G + q0);
                                color = Color.FromArgb(R, G, B);
                                NewImage.SetPixel(k + i * RangeBlock.Y, l + j * RangeBlock.X, color);
                            }
            ...
            NewImage.Save(@"(C:\Users\Dima Bogdanov\Documents\Visual Studio 2017\Projects\Fractal Compression\Fractal Compression\Expanded file.bmp");
        }
